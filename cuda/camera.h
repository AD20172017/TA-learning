...
public:
    ...
    __host__ __device__ void render(const hittable& world) {
        initialize();
...
private:
    ...
    __host__ __device__ vec3 pixel_sample_square() const {
    ...
    __host__ __device__ color ray_color(const ray &r, const hittable &world, int depth) const;
};

__host__ __device__ color camera::ray_color(const ray &r, const hittable &world, int depth) const{
    hit_record rec;
